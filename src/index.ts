...
const productModal = new ViewProductModal(
...
const formContacts = new ViewOrderContactModal(
...
const successModal = new ViewSuccessModal(
...
function getCartProductList(): HTMLElement[] {
    ...
    return items.map((item, index) =>
        new ViewCartProductItem(cloneTemplate(cartItemsTemplate), events).render({
            product: item,
            counter: index + 1,
        })
    );
...
events.on('items:changed', () => {
    const itemsHTMLArray = productList
        .getItems()
        .map((item) =>
...
// Добавлен обработчик изменений корзины
...
// Оставим только вызов метода модели
